...
namespace TicTacToe
{
    ...
    internal class Game
    {
        ...
        public void Start()
        {
            ...
            void SendMessage(string userId, string message) => TelegramBot.Instance.SendMessage(userId, message, buttons: buttons);
                
            if (PlayerOne.State == PlayerInfo.PlayerState.Turn)
            {
                SendMessage(PlayerOne.UserId, baseMessage.Replace("%userId%", PlayerTwo.UserId).Replace("%turn%", "первым"));
                SendMessage(PlayerTwo.UserId, baseMessage.Replace("%userId%", PlayerOne.UserId).Replace("%turn%", "вторым"));
            }
            else
            {
                SendMessage(PlayerTwo.UserId, baseMessage.Replace("%userId%", PlayerOne.UserId).Replace("%turn%", "первым"));
                SendMessage(PlayerOne.UserId, baseMessage.Replace("%userId%", PlayerTwo.UserId).Replace("%turn%", "вторым"));
            }
